<template>
  <div>
    <!-- <h3>HomeContatiner</h3> -->
    <mt-swipe :auto="3000">
      <mt-swipe-item v-for="item in imagesList" :key="item.url">
...
<script>
  export default {
    ...
    methods: {
      getImages() {
        this.$http.get('https://bird.ioliu.cn/v1/?url=http://vue.studyit.io/api/getlunbo').then(result => {
          if (result.body.stats === 0) {
            //成功了
          } else {
            //失败了
          }
